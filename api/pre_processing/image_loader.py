...
def load_and_clean_image(image_bytes: bytes, filename: str) -> np.ndarray:
    """
    Função que verifica a extensão do arquivo e direciona para o decodificador correto. Ambos os
    decodificadores garantem a extração pura dos pixels, eliminando metadados.
    """
...
def _decode_dicom_to_numpy(image_bytes: bytes) -> np.ndarray:
    """
    Função que lida exclusivamente com exames DICOM. Extrai a matriz visual e descarta as tags com
...
def _decode_standard_image_to_numpy(image_bytes: bytes) -> np.ndarray:
    """
    Função que lida com imagens comuns (JPEG, PNG). O método de decodificação ignora inerentemente as tags EXIF/IPTC.
    """
...
def convert_to_clean_png(img_array: np.ndarray) -> bytes:
    """
    Função que exporta a matriz de pixels purificada de volta para bytes no formato PNG. Garante a geração de um arquivo
    final sem qualquer rastro do aparelho de origem.
    """
