...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def forward_features(self, x, attn_mask=None, is_causal=None):
        B = x.shape[0]
...
def Dinov2(args, **kwargs):
    ...
    if args.model_arch == "dinov2_vits14":
        ...
        raise ValueError(
            f"Unknown model_arch '{args.model_arch}'. "
            f"Expected one of: dinov2_vits14, dinov2_vitb14, dinov2_vitl14, dinov2_vitg14"
        )

    model = timm.create_model(arch, pretrained=True, img_size=224, **kwargs)
    return model


def RETFound_dinov2(args, **kwargs):
    model = timm.create_model(
        "vit_large_patch14_dinov2.lvd142m", pretrained=True, img_size=224, **kwargs
    )
    return model
